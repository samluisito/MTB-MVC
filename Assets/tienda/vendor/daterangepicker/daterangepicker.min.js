...
b--;var n,h;if("undefined"===typeof a.startDate&&"undefined"===typeof a.endDate&&e(this.element).is("input[type=text]")){d=e(this.element).val();var f=d.split(this.locale.separator);b=n=null;2==f.length?(b=g(f[0],this.locale.format),n=g(f[1],this.locale.format)):this.singleDatePicker&&""!==d&&(b=g(d,this.locale.format),n=g(d,this.locale.format));null!==b&&null!==n&&(this.setStartDate(b),this.setEndDate(n))}if("object"===typeof a.ranges){for(h in a.ranges)b="string"===typeof a.ranges[h][0]?g(a.ranges[h][0],
this.locale.format):g(a.ranges[h][0]),n="string"===typeof a.ranges[h][1]?g(a.ranges[h][1],this.locale.format):g(a.ranges[h][1]),this.minDate&&b.isBefore(this.minDate)&&(b=this.minDate.clone()),d=this.maxDate,this.dateLimit&&d&&b.clone().add(this.dateLimit).isAfter(d)&&(d=b.clone().add(this.dateLimit)),d&&n.isAfter(d)&&(n=d.clone()),this.minDate&&n.isBefore(this.minDate,this.timepicker?"minute":"day")||d&&b.isAfter(d,this.timepicker?"minute":"day")||(d=document.createElement("textarea"),d.innerHTML=
h,d=d.value,this.ranges[d]=[b,n]);b="<ul>";for(h in this.ranges)b+='<li data-range-key="'+h+'">'+h+"</li>";this.showCustomRangeLabel&&(b+='<li data-range-key="'+this.locale.customRangeLabel+'">'+this.locale.customRangeLabel+"</li>");b+="</ul>";this.container.find(".ranges").prepend(b)}"function"===typeof c&&(this.callback=c);this.timePicker||(this.startDate=this.startDate.startOf("day"),this.endDate=this.endDate.endOf("day"),this.container.find(".calendar-time").hide());this.timePicker&&this.autoApply&&
...
10);var c=this.timePickerSeconds?parseInt(this.container.find(".left .secondselect").val(),10):0;if(!this.timePicker24Hour){var d=this.container.find(".left .ampmselect").val();"PM"===d&&12>b&&(b+=12);"AM"===d&&12===b&&(b=0)}}else b=parseInt(this.container.find(".right .hourselect").val(),10),a=parseInt(this.container.find(".right .minuteselect").val(),10),c=this.timePickerSeconds?parseInt(this.container.find(".right .secondselect").val(),10):0,this.timePicker24Hour||(d=this.container.find(".right .ampmselect").val(),
...
var l=g([d,c,1]),q=g([d,c,a]);c=g(l).subtract(1,"month").month();d=g(l).subtract(1,"month").year();var k=g([d,c]).daysInMonth(),p=l.day();a=[];a.firstDay=l;a.lastDay=q;for(l=0;6>l;l++)a[l]=[];l=k-p+this.locale.firstDay+1;l>k&&(l-=7);p==this.locale.firstDay&&(l=k-6);k=g([d,c,l,12,h,f]);for(d=c=l=0;42>l;l++,c++,k=g(k).add(24,"hour"))0<l&&0===c%7&&(c=0,d++),a[d][c]=k.clone().hour(n).minute(h).second(f),k.hour(12),this.minDate&&a[d][c].format("YYYY-MM-DD")==this.minDate.format("YYYY-MM-DD")&&a[d][c].isBefore(this.minDate)&&
"left"==b&&(a[d][c]=this.minDate.clone()),this.maxDate&&a[d][c].format("YYYY-MM-DD")==this.maxDate.format("YYYY-MM-DD")&&a[d][c].isAfter(this.maxDate)&&"right"==b&&(a[d][c]=this.maxDate.clone());"left"==b?this.leftCalendar.calendar=a:this.rightCalendar.calendar=a;d="left"==b?this.minDate:this.startDate;n=this.maxDate;h="ltr"==this.locale.direction?{left:"chevron-left",right:"chevron-right"}:{left:"chevron-right",right:"chevron-left"};var m='<table class="table-condensed">';m+="<thead>";m+="<tr>";
if(this.showWeekNumbers||this.showISOWeekNumbers)m+="<th></th>";m=d&&!d.isBefore(a.firstDay)||this.linkedCalendars&&"left"!=b?m+"<th></th>":m+('<th class="prev available"><i class="fa fa-'+h.left+" glyphicon glyphicon-"+h.left+'"></i></th>');f=this.locale.monthNames[a[1][1].month()]+a[1][1].format(" YYYY");if(this.showDropdowns){p=a[1][1].month();f=a[1][1].year();l=n&&n.year()||f+5;k=d&&d.year()||f-50;q=f==k;var t=f==l;c='<select class="monthselect">';for(var r=0;12>r;r++)c=(!q||r>=d.month())&&(!t||
r<=n.month())?c+("<option value='"+r+"'"+(r===p?" selected='selected'":"")+">"+this.locale.monthNames[r]+"</option>"):c+("<option value='"+r+"'"+(r===p?" selected='selected'":"")+" disabled='disabled'>"+this.locale.monthNames[r]+"</option>");for(d='<select class="yearselect">';k<=l;k++)d+='<option value="'+k+'"'+(k===f?' selected="selected"':"")+">"+k+"</option>";f=c+"</select>"+(d+"</select>")}m+='<th colspan="5" class="month">'+f+"</th>";m=n&&!n.isAfter(a.lastDay)||this.linkedCalendars&&"right"!=
b&&!this.singleDatePicker?m+"<th></th>":m+('<th class="next available"><i class="fa fa-'+h.right+" glyphicon glyphicon-"+h.right+'"></i></th>');m+="</tr>";m+="<tr>";if(this.showWeekNumbers||this.showISOWeekNumbers)m+='<th class="week">'+this.locale.weekLabel+"</th>";e.each(this.locale.daysOfWeek,function(a,b){m+="<th>"+b+"</th>"});m+="</tr>";m+="</thead>";m+="<tbody>";null==this.endDate&&this.dateLimit&&(h=this.startDate.clone().add(this.dateLimit).endOf("day"),!n||h.isBefore(n))&&(n=h);for(d=0;6>
d;d++){m+="<tr>";this.showWeekNumbers?m+='<td class="week">'+a[d][0].week()+"</td>":this.showISOWeekNumbers&&(m+='<td class="week">'+a[d][0].isoWeek()+"</td>");for(c=0;7>c;c++){h=[];a[d][c].isSame(new Date,"day")&&h.push("today");5<a[d][c].isoWeekday()&&h.push("weekend");a[d][c].month()!=a[1][1].month()&&h.push("off");this.minDate&&a[d][c].isBefore(this.minDate,"day")&&h.push("off","disabled");n&&a[d][c].isAfter(n,"day")&&h.push("off","disabled");this.isInvalidDate(a[d][c])&&h.push("off","disabled");
a[d][c].format("YYYY-MM-DD")==this.startDate.format("YYYY-MM-DD")&&h.push("active","start-date");null!=this.endDate&&a[d][c].format("YYYY-MM-DD")==this.endDate.format("YYYY-MM-DD")&&h.push("active","end-date");null!=this.endDate&&a[d][c]>this.startDate&&a[d][c]<this.endDate&&h.push("in-range");f=this.isCustomDate(a[d][c]);!1!==f&&("string"===typeof f?h.push(f):Array.prototype.push.apply(h,f));f="";k=!1;for(l=0;l<h.length;l++)f+=h[l]+" ","disabled"==h[l]&&(k=!0);k||(f+="available");m+='<td class="'+
f.replace(/^\s+|\s+$/g,"")+'" data-title="r'+d+"c"+c+'">'+a[d][c].date()+"</td>"}m+="</tr>"}m+="</tbody>";m+="</table>";this.container.find(".calendar."+b+" .calendar-table").html(m)},renderTimePicker:function(b){if("right"!=b||this.endDate){var a=this.maxDate;!this.dateLimit||this.maxDate&&!this.startDate.clone().add(this.dateLimit).isAfter(this.maxDate)||(a=this.startDate.clone().add(this.dateLimit));if("left"==b){var c=this.startDate.clone();var d=this.minDate}else if("right"==b){c=this.endDate.clone();
d=this.startDate;var e=this.container.find(".calendar.right .calendar-time div");""!=e.html()&&(c.hour(e.find(".hourselect option:selected").val()||c.hour()),c.minute(e.find(".minuteselect option:selected").val()||c.minute()),c.second(e.find(".secondselect option:selected").val()||c.second()),this.timePicker24Hour||(e=e.find(".ampmselect option:selected").val(),"PM"===e&&12>c.hour()&&c.hour(c.hour()+12),"AM"===e&&12===c.hour()&&c.hour(0)));c.isBefore(this.startDate)&&(c=this.startDate.clone());a&&
c.isAfter(a)&&(c=a.clone())}e='<select class="hourselect">';for(var h=this.timePicker24Hour?23:12,f=this.timePicker24Hour?0:1;f<=h;f++){var g=f;this.timePicker24Hour||(g=12<=c.hour()?12==f?12:f+12:12==f?0:f);var q=c.clone().hour(g),k=!1;d&&q.minute(59).isBefore(d)&&(k=!0);a&&q.minute(0).isAfter(a)&&(k=!0);e=g!=c.hour()||k?k?e+('<option value="'+f+'" disabled="disabled" class="disabled">'+f+"</option>"):e+('<option value="'+f+'">'+f+"</option>"):e+('<option value="'+f+'" selected="selected">'+f+"</option>")}e+=
'</select> : <select class="minuteselect">';for(f=0;60>f;f+=this.timePickerIncrement)h=10>f?"0"+f:f,q=c.clone().minute(f),k=!1,d&&q.second(59).isBefore(d)&&(k=!0),a&&q.second(0).isAfter(a)&&(k=!0),e=c.minute()!=f||k?k?e+('<option value="'+f+'" disabled="disabled" class="disabled">'+h+"</option>"):e+('<option value="'+f+'">'+h+"</option>"):e+('<option value="'+f+'" selected="selected">'+h+"</option>");e+="</select> ";if(this.timePickerSeconds){e+=': <select class="secondselect">';for(f=0;60>f;f++)h=
10>f?"0"+f:f,q=c.clone().second(f),k=!1,d&&q.isBefore(d)&&(k=!0),a&&q.isAfter(a)&&(k=!0),e=c.second()!=f||k?k?e+('<option value="'+f+'" disabled="disabled" class="disabled">'+h+"</option>"):e+('<option value="'+f+'">'+h+"</option>"):e+('<option value="'+f+'" selected="selected">'+h+"</option>");e+="</select> "}this.timePicker24Hour||(e+='<select class="ampmselect">',q=f="",d&&c.clone().hour(12).minute(0).second(0).isBefore(d)&&(f=' disabled="disabled" class="disabled"'),a&&c.clone().hour(0).minute(0).second(0).isAfter(a)&&
(q=' disabled="disabled" class="disabled"'),e=12<=c.hour()?e+('<option value="AM"'+f+'>AM</option><option value="PM" selected="selected"'+q+">PM</option>"):e+('<option value="AM" selected="selected"'+f+'>AM</option><option value="PM"'+q+">PM</option>"),e+="</select>");this.container.find(".calendar."+b+" .calendar-time div").html(e)}},updateFormInputs:function(){this.container.find("input[name=daterangepicker_start]").is(":focus")||this.container.find("input[name=daterangepicker_end]").is(":focus")||
...
this.element.trigger("showCalendar.daterangepicker",this)},hideCalendars:function(){this.container.removeClass("show-calendar");this.element.trigger("hideCalendar.daterangepicker",this)},hoverRange:function(b){this.container.find("input[name=daterangepicker_start]").is(":focus")||this.container.find("input[name=daterangepicker_end]").is(":focus")||(b=b.target.getAttribute("data-range-key"),b==this.locale.customRangeLabel?this.updateView():(b=this.ranges[b],this.container.find("input[name=daterangepicker_start]").val(b[0].format(this.locale.format)),
this.container.find("input[name=daterangepicker_end]").val(b[1].format(this.locale.format))))},clickRange:function(b){this.chosenLabel=b=b.target.getAttribute("data-range-key");b==this.locale.customRangeLabel?this.showCalendars():(b=this.ranges[b],this.startDate=b[0],this.endDate=b[1],this.timePicker||(this.startDate.startOf("day"),this.endDate.endOf("day")),this.alwaysShowCalendars||this.hideCalendars(),this.clickApply())},clickPrev:function(b){e(b.target).parents(".calendar").hasClass("left")?(this.leftCalendar.month.subtract(1,
"month"),this.linkedCalendars&&this.rightCalendar.month.subtract(1,"month")):this.rightCalendar.month.subtract(1,"month");this.updateCalendars()},clickNext:function(b){e(b.target).parents(".calendar").hasClass("left")?this.leftCalendar.month.add(1,"month"):(this.rightCalendar.month.add(1,"month"),this.linkedCalendars&&this.leftCalendar.month.add(1,"month"));this.updateCalendars()},hoverDate:function(b){if(e(b.target).hasClass("available")){var a=e(b.target).attr("data-title"),c=a.substr(1,1);a=a.substr(3,
1);var d=e(b.target).parents(".calendar").hasClass("left")?this.leftCalendar.calendar[c][a]:this.rightCalendar.calendar[c][a];this.endDate&&!this.container.find("input[name=daterangepicker_start]").is(":focus")?this.container.find("input[name=daterangepicker_start]").val(d.format(this.locale.format)):this.endDate||this.container.find("input[name=daterangepicker_end]").is(":focus")||this.container.find("input[name=daterangepicker_end]").val(d.format(this.locale.format));var g=this.leftCalendar,h=this.rightCalendar,
f=this.startDate;this.endDate||this.container.find(".calendar tbody td").each(function(a,b){if(!e(b).hasClass("week")){var c=e(b).attr("data-title");a=c.substr(1,1);c=c.substr(3,1);a=e(b).parents(".calendar").hasClass("left")?g.calendar[a][c]:h.calendar[a][c];a.isAfter(f)&&a.isBefore(d)||a.isSame(d,"day")?e(b).addClass("in-range"):e(b).removeClass("in-range")}})}},clickDate:function(b){if(e(b.target).hasClass("available")){var a=e(b.target).attr("data-title"),c=a.substr(1,1);a=a.substr(3,1);c=e(b.target).parents(".calendar").hasClass("left")?
this.leftCalendar.calendar[c][a]:this.rightCalendar.calendar[c][a];if(this.endDate||c.isBefore(this.startDate,"day")){if(this.timePicker){a=parseInt(this.container.find(".left .hourselect").val(),10);if(!this.timePicker24Hour){var d=this.container.find(".left .ampmselect").val();"PM"===d&&12>a&&(a+=12);"AM"===d&&12===a&&(a=0)}d=parseInt(this.container.find(".left .minuteselect").val(),10);var g=this.timePickerSeconds?parseInt(this.container.find(".left .secondselect").val(),10):0;c=c.clone().hour(a).minute(d).second(g)}this.endDate=
null;this.setStartDate(c.clone())}else!this.endDate&&c.isBefore(this.startDate)?this.setEndDate(this.startDate.clone()):(this.timePicker&&(a=parseInt(this.container.find(".right .hourselect").val(),10),this.timePicker24Hour||(d=this.container.find(".right .ampmselect").val(),"PM"===d&&12>a&&(a+=12),"AM"===d&&12===a&&(a=0)),d=parseInt(this.container.find(".right .minuteselect").val(),10),g=this.timePickerSeconds?parseInt(this.container.find(".right .secondselect").val(),10):0,c=c.clone().hour(a).minute(d).second(g)),
this.setEndDate(c.clone()),this.autoApply&&(this.calculateChosenLabel(),this.clickApply()));this.singleDatePicker&&(this.setEndDate(this.startDate),this.timePicker||this.clickApply());this.updateView();b.stopPropagation()}},calculateChosenLabel:function(){var b=!0,a=0,c;for(c in this.ranges){if(this.timePicker){if(this.startDate.isSame(this.ranges[c][0])&&this.endDate.isSame(this.ranges[c][1])){b=!1;this.chosenLabel=this.container.find(".ranges li:eq("+a+")").addClass("active").html();break}}else if(this.startDate.format("YYYY-MM-DD")==
this.ranges[c][0].format("YYYY-MM-DD")&&this.endDate.format("YYYY-MM-DD")==this.ranges[c][1].format("YYYY-MM-DD")){b=!1;this.chosenLabel=this.container.find(".ranges li:eq("+a+")").addClass("active").html();break}a++}b&&(this.chosenLabel=this.showCustomRangeLabel?this.container.find(".ranges li:last").addClass("active").html():null,this.showCalendars())},clickApply:function(b){this.hide();this.element.trigger("apply.daterangepicker",this)},clickCancel:function(b){this.startDate=this.oldStartDate;
this.endDate=this.oldEndDate;this.hide();this.element.trigger("cancel.daterangepicker",this)},monthOrYearChanged:function(b){b=e(b.target).closest(".calendar").hasClass("left");var a=this.container.find(".calendar."+(b?"left":"right")),c=parseInt(a.find(".monthselect").val(),10);a=a.find(".yearselect").val();!b&&(a<this.startDate.year()||a==this.startDate.year()&&c<this.startDate.month())&&(c=this.startDate.month(),a=this.startDate.year());this.minDate&&(a<this.minDate.year()||a==this.minDate.year()&&
...
timeChanged:function(b){var a=e(b.target).closest(".calendar"),c=a.hasClass("left");b=parseInt(a.find(".hourselect").val(),10);var d=parseInt(a.find(".minuteselect").val(),10),g=this.timePickerSeconds?parseInt(a.find(".secondselect").val(),10):0;this.timePicker24Hour||(a=a.find(".ampmselect").val(),"PM"===a&&12>b&&(b+=12),"AM"===a&&12===b&&(b=0));c?(c=this.startDate.clone(),c.hour(b),c.minute(d),c.second(g),this.setStartDate(c),this.singleDatePicker?this.endDate=this.startDate.clone():this.endDate&&
this.endDate.format("YYYY-MM-DD")==c.format("YYYY-MM-DD")&&this.endDate.isBefore(c)&&this.setEndDate(c.clone())):this.endDate&&(c=this.endDate.clone(),c.hour(b),c.minute(d),c.second(g),this.setEndDate(c));this.updateCalendars();this.updateFormInputs();this.renderTimePicker("left");this.renderTimePicker("right")},formInputsChanged:function(b){b=e(b.target).closest(".calendar").hasClass("right");var a=g(this.container.find('input[name="daterangepicker_start"]').val(),this.locale.format),c=g(this.container.find('input[name="daterangepicker_end"]').val(),
this.locale.format);a.isValid()&&c.isValid()&&(b&&c.isBefore(a)&&(a=c.clone()),this.setStartDate(a),this.setEndDate(c),b?this.container.find('input[name="daterangepicker_start"]').val(this.startDate.format(this.locale.format)):this.container.find('input[name="daterangepicker_end"]').val(this.endDate.format(this.locale.format)));this.updateView()},formInputsFocused:function(b){this.container.find('input[name="daterangepicker_start"], input[name="daterangepicker_end"]').removeClass("active");e(b.target).addClass("active");
e(b.target).closest(".calendar").hasClass("right")&&(this.endDate=null,this.setStartDate(this.startDate.clone()),this.updateView())},formInputsBlurred:function(b){this.endDate||(b=this.container.find('input[name="daterangepicker_end"]').val(),b=g(b,this.locale.format),b.isValid()&&(this.setEndDate(b),this.updateView()))},elementChanged:function(){if(this.element.is("input")&&this.element.val().length&&!(this.element.val().length<this.locale.format.length)){var b=this.element.val().split(this.locale.separator),
a=null,c=null;2===b.length&&(a=g(b[0],this.locale.format),c=g(b[1],this.locale.format));if(this.singleDatePicker||null===a||null===c)c=a=g(this.element.val(),this.locale.format);a.isValid()&&c.isValid()&&(this.setStartDate(a),this.setEndDate(c),this.updateView())}},keydown:function(b){9!==b.keyCode&&13!==b.keyCode||this.hide()},updateElement:function(){this.element.is("input")&&!this.singleDatePicker&&this.autoUpdateInput?(this.element.val(this.startDate.format(this.locale.format)+this.locale.separator+
