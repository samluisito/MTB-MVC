...
trait TClientes {
   ...
   public function insertCliente(
      ...
      if (empty($request)) {
         ...
         $query_insert = "INSERT INTO persona (               	
                        nombres, apellidos,
                        telefono, email_user,
                        sexo,direccionfiscal,localidad,ciudad,pais,
                        password, rolid,oauth_provider,oauth_uid,img) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
   ...
   public function insertPedido(string $transaccionid = NULL, string $datajson = NULL, int $personaid,
           float $subtotal, float $costo_envio, float $monto, string $metodoEntrega, int $tipopagoid, string $direccionenvio, string $status) {
      $this->con = new Mysql();

      $query_insert = "INSERT INTO pedido (transaccionid,
                                          datajson,
                                          personaid,
                                          subtotal,
                                          costo_envio,
   ...
   public function getPedido($idpedido) {
      ...
      if (count($request_ped) > 0) {
         $sql_deta_ped = "SELECT dp.pedidoid,
                                    dp.productoid,
                                    p.nombre,
                                    p.ruta,
                                    dp.precio,
