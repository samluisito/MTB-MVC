...
trait TCategoria
{
    ...
    private function ordanarImagenesBanner($imagenes, $dispositivo)
    {
        foreach ($imagenes as $i => $imagen) {
            if ($imagenes[$i]['img'] === 'banner.png') {
                $imagenes[$i]['url_img'] = DIR_MEDIA . 'images/' . $imagen['img']; //url Img portada
            } else {
                $imagenes[$i]['url_img'] = $dispositivo === 'mobile' ? DIR_IMAGEN . 'thumb_2_' . $imagen['img'] : ($dispositivo === 'tablet' ? DIR_IMAGEN . 'thumb_1_' . $imagen['img'] : DIR_IMAGEN . $imagen['img']);
            }
        }
        return $imagenes;
    }

    public function getBannerT($dispositivo)
    {
        $this->con = new Mysql();
    ...
    public function getMetaCategoriaT(string $ruta)
    {
        $this->con = new Mysql();
        $this->strRuta = $ruta;
